class Solution(object):
    def big_number_sum(self, s1, s2):
        """首先写出两个整数相加和相减的函数，然后根据两个数的正负号分四种类别计算"""
        if s1[0] != '-' and s2[0] != '-':
            self.big_positive_number_sum(s1, s2)
        elif s1[0] == '-' and s2[0] != '-':
            s1 = s1[1:]
            self.big_positive_number_minus(s2, s1)
        elif s1[0] != '-' and s2[0] == '-':
            s2 = s2[1:]
            self.big_positive_number_minus(s1, s2)
        else:
            s1 = s1[1:]
            s2 = s2[1:]
            print('-', end="")
    ...
    def big_positive_number_sum(self,s1, s2):
    ...
    def big_positive_number_minus(self,s1, s2):
